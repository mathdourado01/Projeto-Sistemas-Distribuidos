...
def request_server_list(reference_socket) -> list[str]:
...
def parse_known_servers(raw_value: str) -> dict[str, str]:
    servers = {}
...
def parse_rank_list(server_list: list[str]) -> dict[str, int]:
    result = {}
...
def choose_coordinator_by_rank(ranks: dict[str, int]) -> str:
    if not ranks:
        return ""
